...
# Step 2. extract the locality ####
hotspots <- hotspots[loc, ] |>
    dplyr::select(id, h3, intensity_gi) |>
    sf::st_drop_geometry()
...
x <- x |>
    ...
    dplyr::mutate(total = rowSums(dplyr::across(dplyr::starts_with("2")))) 


gtExtras::gt_plt_summary(x,
...
risk_gua <- risk(betas = denmex::eggs_betas_14_jalisco,
                 ...
                 locality = c("Guadalajara", "Zapopan",
                              "Tonalá", "Tlaquepaque")) |>
    dplyr::select(id, h3, cluster, entomo_cluster, 
                  risk, entomo_risk) |>
...
# Step 3.
gtExtras::gt_plt_summary(risk_gua,
                         title = "Dataset")
...
## [Redefiniendo el Riesgo: Bivariate Maps]{style="color:#003300;"}
<hr style="height:2px;border-width:0;color:#330019;background-color:#330019">
...
source("~/Dropbox/hotspots_vector_2024/3.Functions/bivariate_map.R")
source("~/Dropbox/hotspots_vector_2024/3.Functions/risk.R")
risk_aca <- risk(betas = denmex::eggs_betas_12_guerrero,
             hotspots = hotspots,
             cve_edo = "12",
             locality = "Acapulco de Juarez")
bivariate_map(data = risk_aca, 
                   x_leg = 0.7, 
                   y_leg = 0.65,
                   leg_scale = 0.9,
                   size_lab = 7)
